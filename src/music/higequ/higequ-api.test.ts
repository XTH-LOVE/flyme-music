...
/**
 * The site is PHP-rendered with no JSON API, so parsing is the whole contract.
 * These fixtures are taken from the real page structures.
 *
 * Runs under jsdom (per-file override; the rest of the suite stays on the
 * faster `node` environment) because the parser uses DOMParser.
 */
...
describe('splitHigequArtists', () => {
  ...
  it('按站点约定的 & 拆分多歌手', () => {
    ...
    expect(splitHigequArtists('大头针 Official&俊俊')).toEqual(['大头针 Official', '俊俊']);
  });

  it("西文名中的 ' & ' 不拆分", () => {
    expect(splitHigequArtists('Simon & Garfunkel')).toEqual(['Simon & Garfunkel']);
  });
...
describe('parseHigequSearchHtml', () => {
  ...
  it('无分页容器时保守返回 hasMore=false', () => {
    const { items, hasMore } = parseHigequSearchHtml('<html><body></body></html>');
...
describe('parseHigequPlayerHtml', () => {
  it('解码内联 base64 得到音频直链，并重建 LRC', () => {
    ...
    expect(detail.coverUrl).toBe('https://img1.kuwo.cn/star/albumcover/300/s3s94/93/211513640.jpg');
